...
const { runPrompt, checkProviderAvailable } = require('./ai-provider');
...
function generateContent(account, type, topic, jobId) {
  ...
  const availability = checkProviderAvailable({
    cwd: ROOT,
    provider: account.ai_provider,
    env: process.env,
  });
  ...
  if (!availability.ok) {
    log(jobId, `Provider unavailable (${availability.provider}): ${availability.message.substring(0, 500)}`);
    return false;
  }
