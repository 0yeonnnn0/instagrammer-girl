...
function selectCardTopic(account) {
  ...
  if (topicMode === 'series' && framework) {
    const seriesTopic = `${framework} 공식문서 쉽게 배우기 #${currentPart}/${totalParts}`;
    ...
    if (!loop && currentPart === totalParts) {
      const alreadyPublishedFinal = db.prepare(
        "SELECT 1 FROM topic_history WHERE account_id = ? AND type = 'card' AND topic = ? LIMIT 1"
      ).get(account.id, seriesTopic);
...
function advanceCardSeriesProgress(account, selection, jobId) {
  ...
  if (!loop && finishedFinalPart) {
    ...
    if (nextFramework) {
      db.prepare(
        "UPDATE accounts SET card_series_framework = ?, card_series_current_part = 1, updated_at = datetime('now') WHERE id = ?"
      ).run(nextFramework, account.id);
      if (jobId) {
        log(jobId, `Card series switched: ${selection.framework} ${selection.totalParts}/${selection.totalParts} -> ${nextFramework} 1/${selection.totalParts}`);
      }
      return;
    }
  }
