# Instagram 카드뉴스 & 릴스 생성 프로젝트

> **v6.0** — 카드뉴스 + 릴스 + 바이링구얼(한/영) + studio 템플릿 + Instagram API 직접 업로드

## 프로젝트 개요

이 프로젝트는 Instagram 카드뉴스(캐러셀)와 릴스(숏폼 영상)를 자동으로 생성합니다.
Claude Code가 오케스트레이터 역할을 하며, 리서치 → 카피라이팅 → 렌더링 → 업로드 파이프라인을 실행합니다.

### 콘텐츠 전략 (반드시 준수)

**릴스와 카드뉴스는 다른 주제를 사용합니다.**

| | 릴스 | 카드뉴스 |
|---|---|---|
| **주제** | 오늘의 뉴스·속보·트렌드 | 튜토리얼·팁·정리·가이드 |
| **리서치** | 웹 검색 (최신 뉴스) | 상시 주제 (개발 지식) |
| **역할** | 신규 팔로워 유입 (도달 극대화) | 저장·공유 유도 (팔로워 충성도) |
| **예시** | "Cursor 스캔들 터졌다" | "Git 명령어 TOP 10" |
| **사이즈** | 1080×1920px (9:16) | 1080×1350px (4:5) |
| **업로드** | `upload-reel.js` | `upload-carousel.js` |

같은 주제로 릴스 + 카드뉴스를 동시에 올리지 않습니다. 하루에 릴스 1개 + 카드뉴스 1개를 올리되, 각각 다른 주제를 사용합니다.

### 바이링구얼 (한/영 동시 표기)

모든 콘텐츠는 바이링구얼로 제작합니다:
- 한국어가 메인 (크게)
- 영어가 서브 (작게, 아래에)
- `headline_en`, `emphasis_en` 필드 사용
- 영어 필드가 비어있으면 한국어만 표시 (하위호환)

### 기본 설정

- **템플릿**: `studio` (에디토리얼 디자인)
- **악센트 컬러**: `#C94040`
- **업로드**: Instagram Graph API 직접 (Buffer 미사용)
- **BGM**: `assets/bgm/deep-urban.mp3` (릴스 자동 포함)

**입력**: 주제, 톤, 템플릿 스타일, 슬라이드/씬 수, 악센트 색상
**출력**: `output/{YY-MM-DD-주제명}/` 디렉토리에 PNG + MP4 + 캡션

### 사용 가능한 템플릿 스타일
- **`studio`** — **(기본값)** 에디토리얼 디자인, 비대칭 좌측 정렬, 종이 질감, 얇은 라인, 바이링구얼 지원
...
- `toss` — 토스 스타일 울트라 미니멀, 다크 플랫 배경, 극한의 여백
- `magazine` — 매거진 스타일, 포토 오버레이 + 화이트 클린
- `clean` — 클린 에디토리얼형, 라이트그레이 배경, 그린 하이라이트, 브랜드 마크
- `blueprint` — 블루프린트 프레젠테이션형, 라이트블루그레이 배경, 소프트블루 악센트
...
**출력 폴더 규칙**: `output/{YY-MM-DD-주제명}/` 형식으로 폴더를 생성하여 저장합니다.
- 날짜: 생성 당일 기준 `YY-MM-DD` 형식 (예: `26-03-15`)
- 주제명: 공백은 하이픈(`-`)으로 치환, 한글/영문 가능 (예: `AI-트렌드-2026`)
- 예시: `output/26-03-15-AI-트렌드-2026/`

```bash
# 1. 출력 폴더 생성
mkdir -p output/{YY-MM-DD-주제명}

# 2. 렌더링 실행
node scripts/render.js \
  --slides workspace/slides.json \
  --style {template} \
  --output output/{YY-MM-DD-주제명}/ \
  --accent "{accent_color}" \
  --account "{account_name}"
```

렌더링 완료 후 `output/{YY-MM-DD-주제명}/` 디렉토리에 `slide_01.png` ~ `slide_0N.png` 파일이 생성됩니다.

---

### Step 4.5: 인스타그램 캡션 생성

> 렌더링 완료 후 실행

`workspace/slides.json`과 `workspace/research.md`를 기반으로 인스타그램 게시물 본문(캡션)을 작성하여 `output/{YY-MM-DD-주제명}/text.md` 파일로 저장합니다.

#### text.md 템플릿

```markdown
{훅 문장 — 첫 줄에서 피드 스크롤을 멈추게 하는 한 줄}

{본문 요약 — 카드뉴스 핵심 내용을 2~4문장으로 요약}

{슬라이드별 핵심 포인트 — 번호 이모지로 나열}
1️⃣ {슬라이드 2 핵심}
2️⃣ {슬라이드 3 핵심}
3️⃣ {슬라이드 4 핵심}
...
...
{해시태그 — 주제 관련 15~20개, 대형+중형+소형 믹스}
#{주제관련1} #{주제관련2} ...
#카드뉴스 #인스타카드뉴스 #정보공유 #인사이트
```

#### 캡션 작성 가이드라인

- **훅 문장**: 질문형, 숫자형, 도발형 중 택 1 (예: "2026년, 당신의 직업은 안전할까요?")
- **본문 요약**: 카드뉴스를 넘기지 않아도 핵심을 파악할 수 있되, 넘기고 싶게 만드는 티저 역할
- **슬라이드별 포인트**: 각 content 슬라이드의 핵심을 1줄로 압축, 번호 이모지 사용
- **마무리 인사이트**: 전체 내용을 관통하는 핵심 메시지 1줄
- **CTA**: 저장/공유/팔로우 중 2개 이상 조합
- **해시태그**: 주제 직접 관련 10개 + 일반 도달용 5~10개, 총 15~20개
- **줄바꿈**: 섹션 사이에 빈 줄 1개로 가독성 확보
- **이모지**: 섹션 구분 및 포인트 강조에 적절히 사용 (과용 금지)
- **톤**: 카드뉴스와 동일한 톤(professional/casual/energetic) 유지
...
`output/{YY-MM-DD-주제명}/` 디렉토리의 PNG 파일들을 읽어 다음 항목을 검토합니다:
...
│   ├── magazine/
│   └── clean/
├── templates-reel/      # 릴스 HTML 템플릿 (1080×1920)
│   ├── clean/           # hook.html, point.html, stat.html, list.html,
│   │                    # tip.html, comparison.html, highlight.html, cta.html
│   ├── toss/            # 토스 스타일 (동일 8종)
│   └── bold/            # 볼드 스타일 (동일 8종)
├── scripts/
│   ├── render.js        # Puppeteer HTML → PNG 렌더러 (카드뉴스)
│   ├── render-reel.js   # Puppeteer HTML → PNG/MP4 렌더러 (릴스)
│   └── generate-samples.js
├── workspace/           # 런타임 작업 공간 (research.md, slides.json, reels.json)
├── output/              # 최종 PNG 출력 (YY-MM-DD-주제명 폴더별 저장)
├── config.json          # 기본 설정
└── CLAUDE.md            # 이 파일
```

`workspace/` 디렉토리는 매 생성 시 덮어쓰기됩니다. 이전 결과물은 `output/{YY-MM-DD-주제명}/` 폴더별로 보관됩니다.

---

## 릴스 생성 워크플로우

사용자가 릴스를 요청하면 (예: "AI 트렌드로 릴스 만들어줘") 아래 파이프라인을 실행합니다.
카드뉴스와 동일한 리서치 → 검증 → 카피 → 렌더링 파이프라인을 따르되, 릴스에 최적화된 씬 구조를 사용합니다.
...
| `topic` | (필수) | 릴스 주제 |
| `tone` | `professional` | 톤 (professional / casual / energetic) |
| `reel_template` | `clean` | 릴스 템플릿 스타일 (clean / toss / bold) |
| `scene_count` | `7` | 씬 수 (최소 5, 최대 10) |
| `scene_duration` | `2.5` | 기본 씬 노출 시간 (초) |
| `accent_color` | config 기본값 | 악센트 색상 (hex) |
| `account_name` | config 기본값 | 계정명 |

### 릴스 리서치 & 검증

카드뉴스 Step 2, Step 2.5와 동일한 파이프라인을 사용합니다.

### 릴스 카피라이팅

**출력 파일**: `workspace/reels.json`

#### reels.json 포맷

```json
[
  {"scene": 1, "type": "hook", "badge_text": "꿀팁", "headline": "아직도 이렇게\n코딩하세요?", "subtext": "90%가 모르는 사실", "duration": 2},
  {"scene": 2, "type": "point", "badge_number": "01", "headline": "변수명을 대충 짓는다", "body": "a, b, temp는 이제 그만\n의미 있는 이름을 쓰세요", "duration": 2.5},
  {"scene": 3, "type": "stat", "headline": "코드 리뷰 효과", "emphasis": "85%", "subtext": "버그 감소율", "body": "구글 연구팀에 따르면\n코드 리뷰만으로 버그가 85% 감소", "duration": 3},
  {"scene": 4, "type": "list", "headline": "필수 VS Code 확장", "item1": "Prettier — 코드 포맷팅", "item2": "ESLint — 에러 감지", "item3": "GitLens — Git 히스토리", "item4": "Error Lens — 인라인 에러", "item5": "Copilot — AI 코딩", "duration": 3},
  {"scene": 5, "type": "tip", "badge_text": "💡", "headline": "커밋 메시지 규칙", "body": "feat: 새 기능 추가\nfix: 버그 수정\nrefactor: 리팩토링\n이 규칙만 지켜도 깔끔해집니다", "duration": 3},
  {"scene": 6, "type": "highlight", "headline": "핵심 포인트", "emphasis": "좋은 코드는\n읽기 쉬운 코드다", "body": "미래의 나를 위해\n지금 깔끔하게 작성하세요", "duration": 2.5},
  {"scene": 7, "type": "cta", "headline": "더 많은 개발 팁이\n궁금하다면?", "subtext": "매일 올라오는 개발 인사이트", "cta_text": "팔로우하기", "tag1": "#개발", "tag2": "#코딩", "tag3": "#프로그래밍", "duration": 2}
]
```
...
| 타입 | 사용 시점 | 필드 |
|---|---|---|
| `hook` | 항상 첫 번째 씬 (스크롤 멈추기) | `badge_text`, `headline`, `subtext` |
| `point` | 핵심 포인트 (번호 + 제목 + 설명) | `badge_number`, `headline`, `body` |
| `stat` | 숫자/통계/퍼센트 강조 | `headline` (라벨), `emphasis` (숫자), `subtext` (단위), `body` |
| `list` | 항목 나열 (3~5개) | `headline`, `item1`~`item5` |
| `tip` | 팁/노하우 | `badge_text` (이모지), `headline`, `body` |
| `comparison` | 비교/대조 (Before vs After) | `headline`, `left_title`, `left_body`, `right_title`, `right_body` |
| `highlight` | 핵심 인사이트 강조 | `headline` (라벨), `emphasis` (강조문), `body` |
| `cta` | 항상 마지막 씬 (행동 유도) | `headline`, `subtext`, `cta_text`, `tag1`~`tag3` |

#### 릴스 카피라이팅 가이드라인

- **씬 1 (hook)**: 1.5~2초 안에 스크롤을 멈추게 해야 함. 도발적 질문, 충격적 숫자, "절대 하지 마세요" 등
- **중간 씬**: 한 화면에 1가지 포인트만. 텍스트는 2초 안에 읽을 수 있는 분량
- **duration**: hook은 1.5~2초, 내용이 많은 씬(list, comparison)은 3~3.5초, 일반은 2~2.5초
- **문장 길이**: 한 줄 10자 이내 권장 (릴스는 카드뉴스보다 더 짧게)
- **줄바꿈**: `\n`으로 표현, 한 화면 최대 4줄
- **마지막 씬 (cta)**: 구체적 행동 유도 (팔로우, 저장, 댓글)

### 릴스 카피 토론

카드뉴스 Step 3.5와 동일한 팀 토론 파이프라인을 적용합니다.
추가 평가 기준: 씬 간 읽기 시간 대비 duration 적정성

### 릴스 렌더링

> 카피 토론 통과 후 실행

`render-reel.js` 스크립트를 실행하여 씬 JSON을 PNG 이미지 + MP4 영상으로 변환합니다.

**출력 폴더 규칙**: `output/{YY-MM-DD-주제명}-reel/` 형식

```bash
# 1. 출력 폴더 생성
mkdir -p output/{YY-MM-DD-주제명}-reel

# 2. 렌더링 실행
node scripts/render-reel.js \
  --scenes workspace/reels.json \
  --style {reel_template} \
  --output output/{YY-MM-DD-주제명}-reel/ \
  --accent "{accent_color}" \
  --account "{account_name}"
```

렌더링 결과:
- `scene_01.png` ~ `scene_0N.png` — 개별 씬 이미지 (1080×1920px)
- `concat.txt` — FFmpeg 연결 파일
- `reel.mp4` — 완성 영상 (FFmpeg 설치 시)

**`--png-only` 옵션**: MP4 생성 없이 PNG만 출력. 사용자가 CapCut/InShot에서 직접 편집할 때 유용

### 릴스 캡션 생성

카드뉴스 Step 4.5와 동일하되 릴스 특성 반영:
- 해시태그 20~25개 (릴스는 더 많이)
- CTA에 "이 릴스 저장하고 나중에 다시 보세요" 류 추가
- 파일: `output/{YY-MM-DD-주제명}-reel/text.md`

### 릴스 검토

카드뉴스 Step 5와 동일. 추가 검토 항목:
- Instagram Safe Zone: 상단 250px, 하단 400px 영역에 핵심 콘텐츠가 없는지
- 씬 간 시각적 일관성
...
### clean (릴스)
- 스타일: 클린 에디토리얼형, 라이트그레이 배경
- 기본 악센트: `#8BC34A` (라임 그린)
- 특징: 화이트 카드 UI, 그린 하이라이트, 넓은 여백
- 추천: 교육, 정보 전달, 생산성 팁

### toss (릴스)
- 스타일: 토스 스타일 다크 미니멀
- 기본 악센트: `#3182F6` (토스 블루)
- 특징: 다크 플랫 배경, 극한의 미니멀, 좌측 정렬
- 추천: 테크, 금융, 데이터, 비즈니스

### bold (릴스)
- 스타일: 강렬한 그라디언트 다크
- 기본 악센트: `#6C5CE7` (퍼플)
- 특징: 글로우 효과, 그라디언트 텍스트, 글래스모피즘 카드
...
### 포맷 1: Hook → 리스트 (가장 보편적)
```
hook → point(01) → point(02) → point(03) → cta
```
예: "개발자가 꼭 알아야 할 3가지"

### 포맷 2: 문제 → 해결
```
hook → comparison → point(01) → point(02) → highlight → cta
```
예: "이 습관 바꾸면 생산성 2배"

### 포맷 3: 숫자 임팩트
```
hook → stat → point(01) → point(02) → tip → cta
```
예: "85%가 실패하는 이유"

### 포맷 4: 팁 모음
```
hook → tip → tip → tip → list → cta
```
예: "매일 쓰는 VS Code 단축키 모음"

### 공통 법칙
1. **첫 1.5초**: 스크롤을 멈추게 하는 훅 (질문/숫자/도발)
2. **중간**: 한 화면 = 한 포인트, 빠르게 전환
3. **마지막**: 구체적 CTA (팔로우/저장/댓글)
4. **총 길이**: 15~25초가 최적 (7~10씬)
5. **텍스트**: 짧을수록 좋음. 한 줄 10자 이내
...
```
```
토스 스타일로 "개발자 생산성 팁" 릴스 8장
```
```
볼드 스타일 릴스, "2026 프론트엔드 트렌드", 악센트 #FF6B6B
```
